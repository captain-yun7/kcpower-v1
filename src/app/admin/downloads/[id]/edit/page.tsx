...
export default function EditDownloadPage() {
  ...
  useEffect(() => {
    fetchCategories();
    if (params.id) {
      fetchDownload(params.id as string);
    }
  }, [params.id]);
  ...
  const fetchDownload = async (id: string) => {
    try {
      ...
      if (response.ok) {
        ...
        alert(data.error || '자료를 불러오는데 실패했습니다');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      const response = await fetch(`/api/admin/downloads/${params.id}`, {
        method: 'PATCH',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-2">카탈로그 및 다운로드 자료를 수정합니다</p>
      </div>
